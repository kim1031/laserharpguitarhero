...
void display_leaderboard(char* song) {
  ...
  //strcat(title, song);
  tft.textSetCursor(0, 0); // for title. don't know if this is a good spot or not
//  tft.textEnlarge(1);
  tft.textWrite(title);
//  tft.println(title);
//  tft.println(); //blank line
  //tft.setCursor(50, 50); //play with x and y values
  int place_count = 1;
  char result_str[100];
  char score_str[100];
  int cursor_y;
...
void get_leaderboard(char* song) {
  sprintf(request_buffer, "GET http://608dev.net/sandbox/sc/jgonik/laserharpguitarhero/leaderboards.py?song=This_Love");
